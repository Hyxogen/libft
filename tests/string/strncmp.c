...
#define TEST_ONE(a, b)                                                      \
	do {                                                                \
		...
		assert(sign_or_zero(strncmp(a, b, INT_MAX)) ==              \
		       sign_or_zero(ft_strncmp(a, b, INT_MAX)));            \
		for (size_t __i = 0; __i < __l; ++__i) {                    \
			assert(sign_or_zero(strncmp(a, b, __i)) ==          \
			       sign_or_zero(ft_strncmp(a, b, __i)));        \
			assert(sign_or_zero(strncmp(b, a, __i)) ==          \
			       sign_or_zero(ft_strncmp(b, a, __i)));        \
		}                                                           \
	} while (0)
